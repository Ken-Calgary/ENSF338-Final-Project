...
from myLib.datastructures.trees.AVL import BST
...
if __name__ == '__main__':
    ...
    tree = AVL()
    print(f"Intializing Binary Search Tree with a nothing:\n")
    tree.print_bf()

    tree = AVL(random.randint(0,100))
    ...
    print(f"The tree we're setting (A Binary Search Tree): ")
    set_tree.print_bf() 

    print(f"Setting the AVL to the BST (and balances)")
    ...
    # Testing delete
    print(f"Testing delete with {num} (No Balances)")
    tree.delete(num)
    tree.print_bf()
